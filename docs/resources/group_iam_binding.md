---
# generated by https://github.com/hashicorp/terraform-plugin-docs
page_title: 'workbench_group_iam_binding Resource - workbench'
subcategory: ''
description: |-
  Workbench provisioned group IAM binding resource
---

<!-- markdownlint-disable MD012 MD022 -->

# workbench_group_iam_binding (Resource)

Workbench provisioned group IAM binding resource

## Example Usage

```terraform
resource "workbench_group_iam_binding" "my_iam_binding" {
  group        = data.workbench_group.my_group.group_name
  organization = data.workbench_group.my_group.organization_id
  role         = "MEMBER"
  principals = [
    {
      user = "alice@example.com"
    },
    {
      user = "bob@example.com"
    },
    {
      group = {
        group        = data.workbench_group.another_group.group_name
        organization = data.workbench_group.another_group.organization_id
      }
    }
  ]
}

output "group_bindings" {
  value = workbench_group_iam_binding.my_iam_binding.principals
}
```

<!-- schema generated by tfplugindocs -->

## Schema

### Required

- `group` (String) Workbench managed group name
- `principals` (Attributes Set) List of principals (users, groups, or public) in the IAM binding.
  (see [below for nested schema](#nestedatt--principals))
- `role` (String) Role to be granted to the group. Must be one of the following: `ADMIN`, `OWNER`,
  `READER`, `SUPPORT`

### Optional

- `organization` (String) Workbench organization ID, either UUID or UFID. If it is a UFID, it must
  be prefixed with a tilde (~).

<a id="nestedatt--principals"></a>

### Nested Schema for `principals`

Optional:

- `group` (Attributes) Identifier of a group. (see
  [below for nested schema](#nestedatt--principals--group))
- `public` (Boolean) True if the group is public.
- `user` (String) Email of a user.

<a id="nestedatt--principals--group"></a>

### Nested Schema for `principals.group`

Required:

- `group` (String) Name of the group.

Optional:

- `organization` (String) Workbench organization ID.

## Import

Import is supported using the following syntax:

In Terraform v1.5.0 and later, the
[`import` block](https://developer.hashicorp.com/terraform/language/import) can be used with the
`id` attribute, for example:

```terraform
import {
  to = workbench_group_iam_binding.my_binding
  id = "organizations/{{organization_id}}/groups/{{group_name}}/roles/{{role_name}}"
}
```
